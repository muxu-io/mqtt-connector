...
class MqttConnector:
    ...
    def __init__(self,
        ...
        self.client_id = client_id if client_id else f"mqtt_connector_{uuid.uuid4().hex[:8]}"
        ...
        if self.use_ssl:
            ...
            try:
                if self.verify_ssl:
                    ...
                    self._log('INFO', "SSL/TLS encryption enabled with certificate verification")
                else:
                    ...
                    self.client.tls_set_context(context)
                    self._log('INFO', "SSL/TLS encryption enabled WITHOUT certificate verification (testing mode)")
    ...
    def _log(self, level: str, message: str) -> None:
        """Internal logging method that uses the callback if available.
    ...
    def _setup_callbacks(self) -> None:
        ...
        def on_connect(client, userdata, flags, reason_code, properties=None):
            if reason_code == 0:
                ...
                self._log('ERROR', f"Failed to connect to MQTT broker with reason code {reason_code}")
        ...
        def on_disconnect(client, userdata, flags, reason_code, properties=None):
            ...
            if reason_code != 0:
                self._log('WARNING', f"Unexpected disconnection from MQTT broker (reason: {reason_code})")
                # Start auto-reconnection if enabled
                if self._auto_reconnect and not self._reconnect_task:
                    try:
                        loop = asyncio.get_event_loop()
                        self._reconnect_task = loop.create_task(self._auto_reconnect_loop())
                    except RuntimeError:
                        # No event loop running, auto-reconnection will need to be handled manually
                        self._log('WARNING', "No event loop available for auto-reconnection")
            else:
        ...
        def on_subscribe(client, userdata, mid, reason_codes, properties=None):
            self._log('INFO', f"Subscribed with mid: {mid}, reason codes: {reason_codes}")

        def on_message(client, userdata, msg):
            self._log('DEBUG', f"Received message on topic {msg.topic}: {msg.payload.decode()}")
    ...
    async def connect(self, force_reconnect: bool = False) -> bool:
        ...
        while self.max_reconnect_attempts == -1 or attempts < self.max_reconnect_attempts:
            self._log('INFO', f"Connecting to MQTT broker {self.mqtt_broker}: {self.mqtt_port} (attempt {attempts + 1})")
            ...
            try:
                ...
                result = self.client.connect(self.mqtt_broker, self.mqtt_port, keepalive=60)
                if result == mqtt.MQTT_ERR_SUCCESS:
                    ...
                    if connected:
                        ...
                        self._log('ERROR', "Connection timeout - broker did not respond")
                        self.client.loop_stop()

                else:
                    self._log('ERROR', f"Failed to initiate connection to MQTT broker (error code: {result})")
            ...
            if self.max_reconnect_attempts != -1 and attempts >= self.max_reconnect_attempts:
                self._log('ERROR', f"Maximum reconnection attempts ({self.max_reconnect_attempts}) reached")
                break

            self._log('INFO', f"Retrying connection in {self.reconnect_interval} seconds...")
    ...
    async def publish(self,
        ...
        if now - self._last_publish_time < self.throttle_interval:
            await asyncio.sleep(self.throttle_interval - (now - self._last_publish_time))
        ...
        try:
            ...
            if result.rc == mqtt.MQTT_ERR_SUCCESS:
                ...
                self._log('ERROR', f"Failed to publish message to {topic} (error code: {result.rc})")
                success = False
        ...
        if success:
            ...
            self._log('DEBUG', f"Published message to {topic}")
        else:
    ...
    async def subscribe(self, topic: str, qos: int = 0) -> bool:
        ...
        if not self.connected:
            success = await self.connect()
            if not success:
                self._log('ERROR', f"Failed to subscribe - not connected to broker")
                return False
