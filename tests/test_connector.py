...
async def test_log_callback(mqtt_connector):
    ...
    try:
        ...
        connection_logs = [msg for level, msg in log_messages if "connect" in msg.lower()]
...
async def test_throttling():
    ...
    try:
        ...
        assert elapsed >= 1.0, f"Throttling failed: only took {elapsed: .2f}s for 3 messages"
...
async def test_ssl_auth_publish(ssl_auth_mqtt_connector):
    ...
    try:
        ...
        result = await ssl_auth_mqtt_connector.publish(unique_topic, "SSL + Auth test message")
        assert result is True
    finally:
        await ssl_auth_mqtt_connector.disconnect()
...
async def test_ssl_auth_qos_publish(ssl_auth_mqtt_connector):
    ...
    try:
        ...
        # Test QoS 0
        result = await ssl_auth_mqtt_connector.publish(unique_topic, "SSL + Auth QoS 0 message", qos=0)
        assert result is True

        # Test QoS 1
        result = await ssl_auth_mqtt_connector.publish(unique_topic, "SSL + Auth QoS 1 message", qos=1)
        assert result is True
...
async def test_ssl_auth_context_manager():
    ...
    async with MqttConnector(
        ...
        result = await connector.publish(unique_topic, {
